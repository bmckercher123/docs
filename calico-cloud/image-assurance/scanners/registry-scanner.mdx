...
- If you use the registry scanner with Docker, only tagged images are scanned. However, if you use the scanner with Amazon AWS or Azure credentials, both tagged and untagged images are scanned.
...
The registry scanner supports scanning a registry when the docker credentials for that registry are provided.
Using the environment variables: `DOCKER_USERNAME` and `DOCKER_PASSWORD`, the registry scanner is able to scan images with tags.
This means any untagged images residing in your image registry will not be pulled and scanned.

If you have a valid /.docker/config.json, you can also mount this config file onto the container while running the registry scanner.
```
docker run -e ... -v ~/.docker/config.json:/.docker/config.json quay.io/tigera/image-assurance-registry-scanner:v1.10.4
```

The registry scanner also supports authentication using AZURE and AWS credentials.
...
:::note
If you use the registry scanner with AZURE and AWS credentials, the registry scanner will retrieve images with no tags associated with them.
:::
...
`REGISTRY` - the registry you want to scan. (gcr.io)
`IMAGE_ASSURANCE_API_URL` - Get this URL in the Manager UI.
`IMAGE_ASSURANCE_API_TOKEN` - Get this token in the Manager UI. 
Either the `DOCKER_USERNAME` and `DOCKER_PASSWORD`, Azure credentials, or AWS credentials. 

#### Optional inputs 

`REGISTRY_FILTER` - variable to limit scanning time when you have thousands of repositories and images. Supports a comma-separated list. 
...
gcr.io/dev/api
```
You might not care about the dev "sub" registry. Simply provide -e REGISTRY_FILTER=prod-env,staging-env` to filter out the images under dev.
...
If you have trouble authenticating with the registry, check that the registry credentials are correct.
If you have trouble uploading the results to the {{prodname}}, check that the API token and URL are correct.
