...
import ebpf-sys-reqs from '@site/calico-cloud_versioned_docs/version-3.15/_includes/content/_ebpf-sys-reqs.mdx';
...
This approach is not suitable for AKS with Azure CNI since that platform makes use of the Kubernetes add-on manager. The change will be reverted by the system. For AKS, you should follow [Avoiding conflicts with kube-proxy](#avoiding-conflicts-with-kube-proxy)
below.
