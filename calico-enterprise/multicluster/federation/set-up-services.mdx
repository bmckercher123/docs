...
Use federated services along with federated endpoints to extend and automate endpoints sharing. This feature is optional if you have your own service discovery mechanism.
...
A federated service looks similar to a regular Kubernetes service; instead of using a pod selector, it uses an annotation. For example:
...
- [Cluster mesh is set up](../federation/set-up-cluster-mesh)
...
The Tigera Federated Services Controller manages this service, populating the service endpoints from all of the services that match the service selector configured in the annotation.
